...
Turn an implementation idea into approved project contracts and one concise persistent goal. Do not start production implementation during this workflow.
...
1. Inspect the coordinating repository, every affected repository, applicable `AGENTS.md` files, relevant design/specification/goal documents, and the code and tests needed to verify current facts. Inspect persistent goal state before changing an existing goal file. If goal status cannot be inspected, stop before editing that file or creating a replacement and resume only after status can be verified.
2. Identify the real requirement owners. Before editing, show the user which existing documents will change, whether a dedicated implementation specification is needed, where the goal will live, and why each choice follows the reference contract.
...
6. Update the approved owner documents. Create a dedicated implementation specification only when the reference contract requires one.
...
8. When semantic review passes, create or update the dated goal file defined by `Goal File` in the required reference. Semantically reread it with its source contracts, then show the resulting document set and diff. Do not create an implementation plan, commit, push, or begin implementation.
...
- If an owner conflict remains unresolved, stop without conflicting edits or a goal file and report the exact decision still required.
- For either `/goal` fallback, print this exact semantic command in the user's language:

```text
/goal When no other goal is unfinished, implement the objective in <goal-path>. Treat that file as the complete completion contract; finish its full scope and verification.
```
