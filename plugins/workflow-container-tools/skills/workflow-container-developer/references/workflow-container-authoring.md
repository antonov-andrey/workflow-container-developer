...
| `WorkflowStepCodexConcurrentBase` | Детерминированный bounded запуск упорядоченного списка независимых invocations через `DBOS.run_step_async` с сохранением порядка результатов | Те же предметные hooks, что и `WorkflowStepCodexBase`; отдельный scheduler не реализуется |

Конкретный шаг не переопределяет финальный `run(...)`, `run_list(...)` или внутреннюю диспетчеризацию жизненного цикла. Базовые классы содержат реальное общее поведение, а не только сигнатуры. Полные сигнатуры приведены в `Приложение A. Публичные интерфейсы`.
...
Для `WorkflowStepCodexConcurrentBase` конкретный async workflow передает упорядоченный список `WorkflowStepInvocation` в унаследованный `run_list(...)`. Общий base проверяет, что список принадлежит одному run и содержит разные каталоги шагов, затем создает checkpointed calls через `DBOS.run_step_async` и применяет run-local bound; concrete workflow не реализует собственный semaphore, worker pool или динамическую DBOS queue. Base планирует и ожидает весь список даже при ошибке одного элемента, после чего поднимает первую ошибку в порядке `invocation_list`. Успешные вызовы остаются checkpointed и при восстановлении не выполняют внешнюю работу повторно.
...
Все Pydantic-модели в этом блоке используют строгую конфигурацию. `WorkflowInputBase`, config, context, capability, policy, `SqliteStateTable` и `WorkflowStepInvocation` дополнительно используют `frozen=True`; их поля нельзя менять после создания. `WorkflowStepCodexState` остается изменяемым состоянием. Конкретные input-модели являются полными снимками: создание без обязательного значения или с дополнительным полем завершается validation error.
...
`WorkflowStepCodexConcurrentBase.run_list(...)` вызывается только из async DBOS workflow. Он принимает непустой упорядоченный список независимых invocation objects и до запуска требует один общий `result_dir`, один общий `workflow_input_path`, уникальные `step_instance_dir` и containment каждого каталога шага внутри того же run. Затем он создает checkpointed вызовы через `DBOS.run_step_async` в порядке списка, одновременно допускает не более `workflow_step_config.concurrency` вызовов и возвращает результаты в исходном порядке. Он не изменяет DBOS queue и не допускает одновременное выполнение зависимых invocation objects. Ошибка одного элемента не прекращает планирование остальных: base выполняет весь список под тем же bound, ожидает каждый результат и затем поднимает ошибку элемента с наименьшим индексом. Успешные checkpointed результаты переиспользуются при восстановлении.
...
| `workflow_container_runtime/step/base.py` | `StepResultValidationError`, `WorkflowStepBase` и `WorkflowStepDeterministicBase`. |
...
| `workflow_container_runtime/step/codex.py` | Config bases шагов Codex, `WorkflowStepCodexRuntimePolicy`, `WorkflowStepCodexState`, `WorkflowStepInvocation`, `WorkflowStepCodexBase` и `WorkflowStepCodexConcurrentBase`. |
