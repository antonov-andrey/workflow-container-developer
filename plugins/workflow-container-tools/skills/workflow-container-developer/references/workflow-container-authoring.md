...
| --- | --- |
| `workflow-control-center` | Платформа выполнения: создание и публикация `WorkflowSourceVersion`, exact-source build, обязательная проверка image, schema snapshots, формы `Workflow` и `WorkflowRun`, единая файловая система Data, immutable revisions и manifests, Athena/Glue projection, `WorkflowRunController`, run-local control service, runtime capabilities и состояние запуска. |
| `workflow-container-contract` | Независимые от среды выполнения модели и загрузчики `workflow.yaml`, включая build, command, publisher tests, Data paths, secrets, datasets, steps и capabilities, `versions.yaml`, ограниченный профиль `input.schema.json`, input validation, определения input migrations, `WorkflowRunContext`, control requests, `WorkflowRunStatus`, `McpPlaywrightProfileWritebackPolicy` и общий `WorkflowResult` для границы с платформой. |
| `workflow-container-runtime` | Необязательная общая реализация платформенного интерфейса: базовые классы workflow и конфигураций, адаптер стандартной среды и control protocol, Data paths, построитель control requests, жизненный цикл, run-local concurrency и browser-profile lease, стандартные пути, атомарная запись JSON и JSON Lines, SQLite current state, запуск Codex, рендеринг общих prompt, материализация и общие валидаторы. |
...
`browser_vpn_runtime.config.is_vpn_enabled` задается явно и не имеет скрытого значения по умолчанию. При `true` `secret_key_list` содержит два разных каталога строго в порядке browser profile, затем OpenVPN; выбранный OpenVPN `Secret` должен содержать `config.json` и указанный им `.ovpn` файл. При `false` список содержит только browser profile, платформа не требует и не монтирует OpenVPN `Secret`, не создает VPN gateway и запускает browser с прямым egress. Переключение режима требует новой успешно протестированной `WorkflowSourceVersion`; это не настройка отдельного `WorkflowRun`.
...
Процесс workflow, установка зависимостей, DBOS и Codex используют обычный сетевой путь. При `browser_vpn_runtime.config.is_vpn_enabled: true` только `vpn-egress` gateway владеет OpenVPN, `tun0` и его сетевым namespace, а Playwright работает в отдельном namespace и обращается к gateway через SOCKS. При `false` gateway отсутствует, Playwright использует прямой egress. В обоих режимах workflow получает одинаковый готовый MCP URL.
...
После успешной semantic verification именованного профиля общий runtime, не освобождая profile lease, выполняет `POST <mcp_playwright_profile_writeback_candidate_url>?profile=<physical-profile>` с versioned JSON body, содержащим stable step identity и transition identity, и явным runtime-owned конечным положительным тайм-аутом. URL уже привязан платформой к текущему `WorkflowRun`, exact capability snapshot и назначенному secret path; runtime не передает эти значения, policy или filesystem path повторно. Run-local control endpoint сериализует candidate transitions этой scope и проверяет сохраненную policy. Пустой список статусов или профиль, отфильтрованный по имени, дает успешный no-op без обращения к browser router.
...
Отсутствие candidate допустимо при отключенной policy, отсутствующей browser capability, использовании только изолированных профилей, отсутствии invocation с подходящим именованным профилем или до первого успешного подходящего invocation при `working`. Факт использования именованного профиля определяется существованием его каталога только под `/runtime/mcp_playwright_profile/profile/`; отдельный usage registry не создается. Принятый image-authored final transition завершается ошибкой только тогда, когда его состояние требует writeback, существует хотя бы один каталог физического профиля, подходящий под policy, но ни одна подходящая invocation не завершилась успешно и candidate не существует. Infrastructure failure или platform cancellation без принятого final intent не выполняют эту проверку. Параллельные непересекающиеся scopes принимаются независимо; пересекающийся writeback с устаревшей expected revision завершается conflict и не заменяет concurrent result.
